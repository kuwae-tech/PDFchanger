...
const assetsMplPath = path.join(assetsLicensesDir, "MPL-2.0.txt");
...
const MPL_TEXT = `Mozilla Public License Version 2.0
==================================
...
Covered Software is authorized under this disclaimer.
...
Secondary Licenses under the terms of this License, the notice described
...
async function ensureAssetsLicenses(): Promise<void> {
  await fs.ensureDir(assetsLicensesDir);
  if (!(await fs.pathExists(assetsMplPath))) {
    await fs.writeFile(assetsMplPath, MPL_TEXT, "utf8");
  }
}
